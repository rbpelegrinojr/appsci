...
$resInfo = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM teachers_tbl WHERE teacher_id = '$uid'"));
$rS = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM selected_group_tbl WHERE teacher_id = '{$resInfo['teacher_id']}'"));

if ($username != $resInfo['username']) {
	$username = null;
    $username = '';
    session_destroy();
    header('location: login_view.php');
...
<nav class="sidebar dark_sidebar">
	...
	<ul id="sidebar_menu">
		<li class="">
			<a href="index.php" aria-expanded="false">
				<div class="nav_icon_small">
					...
					<!-- <img src="https://demo.dashboardpack.com/user-management-html/img/menu-icon/12.svg" alt=""> -->
				</div>
				<div class="nav_title">
		...
		<?php
		if ($nSG = mysqli_num_rows(mysqli_query($con, "SELECT * FROM selected_group_tbl WHERE teacher_id = '{$resInfo['teacher_id']}'")) > 0) {
			?>
			<li class="">
				...
				<ul>
					<?php
					$qSG = mysqli_query($con, "SELECT * FROM selected_group_tbl WHERE teacher_id = '{$resInfo['teacher_id']}'");
					while ($rSG = mysqli_fetch_assoc($qSG)) {
						$rGroup1 = mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM groups_tbl WHERE group_id = '{$rSG['group_id']}'"));
						?>
						<li><a href="view_group.php?s_id=<?php echo $rSG['selected_group_id']; ?>"><?php echo $rGroup1['group_name']; ?></a></li>
						<?php
					}
					?>
				</ul>
