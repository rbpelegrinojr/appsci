		...
		<div class="white_card_body">
			...
			<div class="table-responsive">
				<table class="table" id="myTable">
					...
					<tbody>
						<?php
						$query = mysqli_query($con, "SELECT * FROM research_tbl WHERE research_status = '1' OR research_status = '2'");
						while ($row = mysqli_fetch_assoc($query)) {
							...
							<tr>
								...
								<td><a href="#" onclick="approveResearch(<?php echo $row['research_id']; ?>);"><span class="badge rounded-pill bg-primary">Approve</span></a> | <a href="#" onclick="disapproveResearch(<?php echo $row['research_id']; ?>);"><span class="badge rounded-pill bg-warning">Disapprove</span></a></td>
							</tr>
							<?php
						}
						?>
...
<script type="text/javascript">
	...
	function approveResearch(r_id) {
		if (confirm('Approve Reseach Paper?')) {
			window.location.href='controller/save_data.php?r_id='+r_id+'&btnApproveResPaper=1';
		}
	}
	function disapproveResearch(r_id) {
		if (confirm('Approve Reseach Paper?')) {
			window.location.href='controller/save_data.php?r_id='+r_id+'&btndisapproveResPaper=1';
		}
	}
</script>
